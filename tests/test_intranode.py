...
def test_main(
    ...
    dispatcher = FuscoMoEDispatcher(num_local_experts, local_expert_indices, num_ranks, world_group, fusco)
...
def test_loop(local_rank: int, num_local_ranks: int, args: argparse.Namespace):
    ...
    global fusco
    fusco = FUSCO(group_ranks=[list(range(num_ranks))], library_path=os.path.join(rootdir, "lib", "libfusco.so"))
    torch.manual_seed(global_rank)

    test_main(args,local_rank, num_ranks, global_rank, group)
