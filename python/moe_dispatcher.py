...
class FuscoMoEDispatcher:
    def __init__(
        self, num_local_experts: int, local_expert_indices: list[int], ep_size: int, ep_group: torch.distributed.ProcessGroup, fusco: FUSCO
    ):
    ...
    def preprocess(self, indices: torch.Tensor) -> torch.Tensor:
        ...
        num_local_tokens_per_rank = num_local_tokens_per_expert.view(self.ep_size, self.num_local_experts).sum(dim=1)
        ...
        num_global_tokens_per_expert = (
            gather_along_first_dim(num_local_tokens_per_expert, self.ep_group)
        ...
        if self.num_ep_tokens > 0:
            self.recvindices = idxtools.indices_gen(num_global_tokens_per_local_expert, num_tokens_per_local_expert, num_tokens_per_ep, self.num_ep_tokens.item())
        else:
...
class Fusco2DMoEDispatcher:
    def __init__(self, num_local_experts: int, local_expert_indices: list[int], ep_size: int, ep_group: torch.distributed.ProcessGroup, global_fusco: FUSCO, intra_fusco: FUSCO, num_local_ranks: int = 8):
        self.num_local_experts = num_local_experts
        self.num_experts = ep_size * num_local_experts
        assert self.num_local_experts > 0, "Expected at least one expert"
        self.local_expert_indices = local_expert_indices
    ...
    def preprocess(self, indices: torch.Tensor, probs: torch.Tensor) -> torch.Tensor:
        ...
        assert topk > 1, "2D Dispatcher is efficient only when topk > 1, please use FuscoMoEDispatcher instead"
        ...
        node_expert_begin, node_expert_end = num_experts_per_node * my_node, num_experts_per_node * (my_node + 1)
        device = torch.cuda.current_device()

         # [ep_size * seqlen, topk]
        global_indices = (
            gather_along_first_dim(indices, self.ep_group)
            .reshape(self.ep_size, seqlen, topk)
        )
        global_probs = (
            gather_along_first_dim(probs, self.ep_group)
            .reshape(self.ep_size, seqlen, topk)
        )
        ...
        intergroup_indices = intragroup_indices // num_experts_per_node * ranks_per_node + (my_rank % ranks_per_node)

        # [nNodes, seqlen, ep_size]
        intergroup_mapping = torch.zeros(size=[nnodes, seqlen, self.ep_size], dtype=torch.int64, device=device)
        ...
        pos_mapping_s1 = torch.repeat_interleave(torch.arange(seqlen, device=device), self.intergroup_expanded_size)
        self.sendindices_s1 = pos_mapping_s1[self.backindices_s1]

        # ========================= stage 2 =========================
        # [ep_size, seqlen, num_experts]
        global_mapping = torch.zeros(size=[self.ep_size, seqlen, self.num_experts], dtype=torch.int64, device=device)
        ...
        intragroup_mask = (intragroup_indices >= node_expert_begin) & (intragroup_indices < node_expert_end)
        ...
        # [8, num_experts]
        group_send_tokens_per_expert_s2 = num_global_tokens_per_expert.reshape(nnodes, ranks_per_node, -1).sum(dim=0)
        # [num_node_experts]
        send_tokens_per_expert_s2 = group_send_tokens_per_expert_s2[local_rank, node_expert_begin : node_expert_end]
        # [8]
        self.send_splits_s2 = send_tokens_per_expert_s2.view(ranks_per_node, self.num_local_experts).sum(dim=1).to(torch.device("cpu"))
        # int
        self.send_tokens_s2 = self.send_splits_s2.sum()

        self.backindices_s2 = torch.argsort(mask_indices, stable=True)
        pos_mapping_s2 = torch.repeat_interleave(torch.arange(self.intragroup_expanded_size.numel(), device=device), self.intragroup_expanded_size)
        self.sendindices_s2 = pos_mapping_s2[self.backindices_s2]
        
        # [8, num_local_experts]
        group_recv_tokens_per_expert_s2 = group_send_tokens_per_expert_s2[:, self.local_expert_indices[0] : self.local_expert_indices[-1] + 1].contiguous()
        # [num_local_experts]
        ...
        if self.recv_tokens_s2 > 0:
            self.recvindices_s2 = idxtools.indices_gen(group_recv_tokens_per_expert_s2, recv_tokens_per_expert_s2, recv_tokens_per_rank_s2, self.recv_tokens_s2.item())
        else:
            self.recvindices_s2 = torch.empty(0, dtype=torch.int64, device=device)

        assert self.send_tokens_s2 == self.intragroup_probs.numel(), f"Mismatch: {self.send_tokens_s2} vs. {self.intragroup_probs.numel()}, this may be caused by duplicate experts selected in top-k (i.e., a token selecting the same expert multiple times)."
    ...
    def combine(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if self.send_tokens_s2 > 0:
            ...
            outputs_buffer = torch.segment_reduce(outputs_buffer, lengths=self.intragroup_expanded_size, reduce='sum')
        ...
        if self.send_tokens_s1 != self.intergroup_expanded_size.numel():
            outputs_unpermuted = torch.segment_reduce(outputs_unpermuted, lengths=self.intergroup_expanded_size, reduce='sum')
        return outputs_unpermuted